...
import 'package:fruit_classifier/DialogBox/request.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  String modelUrl = 'http://13.232.29.100/predict';
  ...
  void predFruit(File imageFile) async {
    ...
    try {
      ...
      //   )
      // });
      // final response = await dio.post("path", data: formData).asStream();
      // response.
  ...
  Future getImage(String source) async {
    ...
    if (status.isGranted) {
      ...
      // print("denied");
      showRequestDialog(context, source);
    } else {
      showRequestDialog(context, source);
    }
  }
